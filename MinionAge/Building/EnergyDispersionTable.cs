...
namespace DebuffRoulette
{
    ...
    public class EnergyDispersionTable : KMonoBehaviour
    {
        ...
        private void CheckOccupant()
        {
            ...
            if (prefabID != null)
            {
                if (prefabID.HasTag("KmodMiniBrainCore2")) // 是不是大脑
                {
                    HandleEdibleItem();
                }
            ...
            foreach (var entry in pickupableItemsInRange)
            {
                ...
                if (pickupable != null)
                {
                   ...
                   // AudioUtil.PlaySound(ModAssets.Sounds.SPLAT, ModAssets.GetSFXVolume() * 0.5f, 1f);
                    AudioUtil.PlaySound(ModAssets.Sounds.DOORBELL, CameraController.Instance.GetVerticallyScaledPosition(pickupable.gameObject.transform.GetPosition()), 1f);
