...
{
    public static class SpecializationHeatWanderer
    {
        // 默认检测半径
        ...
        public static void TriggerScan(GameObject minion, int emissionRadius = DefaultEmissionRadius)
        {
            ...
            float bufftime = KModDeBuff.GetEffectRemainingTime(minion, "HeatWanderer");
            if (bufftime <= 0 || prefabID.HasTag("Corpse"))
            {
                KModDeBuff.RemoveEffect(minion, "HeatWanderer");
                return;
            }
