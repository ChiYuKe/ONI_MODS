...
{
    public class SpecializationCoolWanderer 
    {
        // 默认检测半径
        ...
        public static void TriggerScan(GameObject minion, int emissionRadius = DefaultEmissionRadius)
        {
            ...
            if (!prefabID.HasTag(KGameTags.CoolWanderer))
            {
            ...
            float bufftime = KEffects.GetEffectRemainingTime(minion, KEffects.ETERNALDECAY_COOLWANDERER);
            if (bufftime <= 0 || prefabID.HasTag(GameTags.Corpse))
            {
                KEffects.RemoveBuff(minion, KEffects.ETERNALDECAY_COOLWANDERER);
                return;
            }
