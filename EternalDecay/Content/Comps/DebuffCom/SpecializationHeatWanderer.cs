...
{
    public class SpecializationHeatWanderer 
    {
        // 默认检测半径
        ...
        public static void TriggerScan(GameObject minion, int emissionRadius = DefaultEmissionRadius)
        {
            ...
            if (!prefabID.HasTag(KGameTags.HeatWanderer))
            {
                return;
            }

            // 检查 HeatWanderer Buff 是否存在
            float bufftime = KEffects.GetEffectRemainingTime(minion, KEffects.ETERNALDECAY_HEATWANDERER);
            if (bufftime <= 0 || prefabID.HasTag(GameTags.Corpse))
            {
                KEffects.RemoveBuff(minion, KEffects.ETERNALDECAY_HEATWANDERER);
                return;
            }
